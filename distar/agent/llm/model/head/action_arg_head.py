...
class DelayHead(nn.Module):
    ...
    def forward(self, embedding, delay: Optional[torch.Tensor] = None,seed = None):
        if seed == None:
...
class QueuedHead(nn.Module):
    ...
    def forward(self, embedding, queued=None,seed=None):
        if seed == None:
            seed = random.randint(0, 10000)
        torch.manual_seed(seed)
        x = self.fc1(embedding)
        ...
        if selected_units is not None and selected_units_num is not None:  # train
            ...
            for i in range(self.max_select_num):
                ...
                if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                    ...
                    indices = torch.arange(bs).to(device)  # Move indices to the same device as selected_units_one_hot
                    indices = indices[~end_flag]
                    result = result.to(device)  # Move result to the same device
                    selected_units_one_hot[indices, result[~end_flag], :] = 1
                    if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
...
class LocationHead(nn.Module):
    ...
    def forward(self, embedding, map_skip: List[Tensor], location=None,seed=None):
        if seed == None:
            seed = random.randint(0, 10000)
        torch.manual_seed(seed)
