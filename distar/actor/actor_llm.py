...
class Actor(object):
    def __init__(self, cfg):
        ...
        self.input = ""
        self._input_latency = 0 #gameloops
            ...
            while episode_count < self._cfg.episode_num:
                try:
                    ...
                    while True:  # one episode loop
                        if pipe_c is not None and pipe_c.qsize() > 0:
                            cmd = pipe_c.get()
                            if cmd == 'reset':
                                break
                            elif cmd == 'close':
                                ...
                                self.input = cmd
                                print(f"Received new input: {self.input}")
                                self._input_latency = 10
                        ...
                        for player_index, obs in players_obs.items():
                            ...
                            if self._job_type == 'train':
                                self.agents[player_index]._model_last_iter = self._comm.model_last_iter_dict[
                                    player_id].item()
                            if self._cfg.agents.get(player_id) == 'llm':
                                if self.agents[player_index].ask_llm == True and self._input_latency > 0:
                                    actions[player_index] = self.agents[player_index].step(obs,self.input)
                                else:
                        ...
                        if self._input_latency > 0:
                            self._input_latency -= 1
                        # env step
                        ...
                        if not done:
                            ...
                            if 'test' in self._whole_cfg and self._whole_cfg.test.get('tb_stat', False):
                                ...
                                path = os.path.join(self._env._result_dir,
                                                    '{}_{}_{}_.json'.format(env_id, episode_count, player_index))
    ...
    def run(self, pipe=None):
        torch.set_num_threads(1)
        if "init_dist" in self._whole_cfg.common and self._whole_cfg.common.init_dist:
            dist_init(self._whole_cfg.dist, "actor")

        if self._job_type == "train":
            ps = []
            for i in range(self._cfg.env_num):
                job = self._comm.job if hasattr(self, '_comm') else {}
                p = mp.Process(target=self._inference_loop, args=(i, job, None, pipe))
                p.start()
                ps.append(p)
            for p in ps:
                p.join()

        elif "test" in self._job_type:
            self._inference_loop(pipe_c=pipe)
